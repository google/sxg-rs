...
export async function spawnSxgServer({
  ...
  // Creates an SXG, and puts it into sxgList, and returns the index.
  // Signs subresources recursively if the remaining subresource level is
  // positive.
  async function createSxgIntoList(
    innerUrl: string,
    certOrigin: string,
    subresourceLevel: number
  ) {
    ...
    if (subresourceLevel > 0) {
      const subresourceUrls = Array.from(urlRecorder.visitedUrls());
